...
#### Complete ImageSetConfiguration for this installation
  ...
  mirror:
    ...
    operators:
      ...
      - catalog: registry.redhat.io/redhat/redhat-operator-index:v4.21
        packages:
          ...
          # for GPFS kernel module lifecycle
          - name: kernel-module-management

          # Node networking:
          # bridges, bonds, VLANs, secondary VM networks
          - name: kubernetes-nmstate-operator
      ...
      # This is NOT IBM Fusion Software and NOT Fusion Data
      # Foundation.
      # ---------------------------------------------------------
    ...
    #
    # Required by Fusion Access for SAN.
    #
    # These are IBM Storage Scale / GPFS runtime components,
    # not IBM Fusion Software or Fusion Data Foundation.
    # =========================================================
    additionalImages:
  ...
  mirror:
    ...
    additionalImages:
      ...
      - name: cp.icr.io/cp/gpfs/ibm-spectrum-scale-gui:v6.0.1.0@sha256:fd85ae
      - name: cp.icr.io/cp/gpfs/ibm-spectrum-scale-coredns:v6.0.1.0@sha256:781db6ee6019ae2468b57f48abc17033ccfcdb88beb555b2797ee4fe32a2731b
      ...
      # ---------------------------------------------------------
      # UBI images used by Fusion Access / KMM / Scale workflows
      # ---------------------------------------------------------
...
Requires internet access.On a machine connected to the internet, you run **oc-mirror** targeting a local directory on your portable media (e.g., a mounted USB drive) instead of a registry URL.
  ...
  mkdir -p "${HOME}/oc-misnapshotter:v8.5.0@sha256:da081c27e8a6d91f36042c1942362d0515ced8d06e18c11b8f893e58c4d6d797
