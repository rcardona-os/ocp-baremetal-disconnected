...
# Disconnected OpenShift 4.21.26 Bare-Metal Installation

## Agent-based Installer — Static IP — Existing DNS — OpenShiftManagedDefault
...
| NTP                   | —                                 | `192.168.50.11` |
| API VIP               | `api.ocpcluster.example.com`      | `192.168.50.20` |
| Internal API VIP      | `api-int.ocpcluster.example.com`  | `192.168.50.20` |
| Ingress VIP           | `*.apps.ocpcluster.example.com`   | `192.168.50.21` |
| Master 0 / Rendezvous | `master-0.ocpcluster.example.com` | `192.168.50.31` |
| Master 1              | `master-1.ocpcluster.example.com` | `192.168.50.32` |
| Master 2              | `master-2.ocpcluster.example.com` | `192.168.50.33` |
...
Before creating the ISO, verify DNS from the installation/bastion host.
...
master-1 -> 192.168.50.32
master-2 -> 192.168.50.33

worker-0 -> 192.168.50.41
worker-1 -> 192.168.50.42
worker-2 -> 192.168.50.43
```

Verify reverse records:
...
The Agent-based Installer documentation requires the API, application wildcard and node DNS to be resolvable as part of the cluster network design.
...
Replace `<INTERFACE>` with the interface connected to `192.168.50.0/24`.
...
During discovery/bootstrap all hosts communicate with the Assisted Service running on the rendezvous host over TCP/8090. The port is only required during installation.

Also ensure the normal OpenShift machine-network communication is not blocked, including access to:
